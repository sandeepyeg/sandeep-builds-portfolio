...
export const PERSONAL_PROJECTS: readonly PersonalProject[] = [
  ...
  },
  {
    ...
    role: 'Full-stack admin platform demonstrating authenticated workflows, member profiles, role management, API contracts, and practical UI state handling.',
    details: [
      'Members Management Platform is a full-stack application built around common business administration workflows: authentication, member profiles, role management, and CRUD operations over structured member data.',
      'The backend API provides the protected contract for member data. It is responsible for validation, persistence, authorization boundaries, and consistent responses so the frontend is not forced to guess how the system behaves.',
      'The Angular SPA focuses on practical screens: lists, forms, profile views, edit flows, loading states, and error handling. A useful admin app has to be quick to scan and predictable during repeated use.',
      'The project is a good example of clean separation across the stack. The frontend handles interaction and presentation, while the API owns rules, data access, authentication, and business operations.',
      ...
      'The project also gives room to demonstrate maintainable full-stack structure. Shared response shapes, predictable error contracts, form-level validation, route guards, and focused API services make the application easier to reason about as it grows.',
    ...
    flow: ['User', 'Angular SPA', 'Auth Guard', '.NET API', 'Member Service', 'PostgreSQL'],
    architecture: [
      'Angular SPA with protected routes, forms, profile pages, and member lists',
      'Authentication and authorization checks before sensitive workflows',
      '.NET API for member operations, validation, and response contracts',
      'Service layer for business rules and role-aware operations',
      'PostgreSQL persistence for users, members, roles, and related records',
    ],
    keyPoints: [
      'Authentication and member profile workflows',
      'Role-aware administrative screens',
      'Separation between frontend and backend responsibilities',
      'Practical CRUD and validation patterns',
    ],
    demoValue: [
    ...
    nextSteps: [
      'Add screenshots and a guided demo account',
      'Expand API tests around authorization and validation',
      'Document setup, environment variables, and database migrations',
    ],
  },
  {
